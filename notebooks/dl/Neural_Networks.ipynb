{
 "cells": [
  {
   "cell_type": "markdown",
   "id": "8a2875cc-f6e0-43cc-ab57-e1184dcc4a55",
   "metadata": {},
   "source": [
    "# Neural Networks\n",
    "\n",
  ...
  {
   "cell_type": "markdown",
   "id": "8eabb7f8-3327-4cba-a024-a8d3701f21b1",
   "metadata": {},
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "014747ce-e334-48b2-abbd-4f391ed3675e",
   "metadata": {},
   "source": [
    "### Perceptron\n",
    "\n",
  ...
  {
   "cell_type": "markdown",
   "id": "9ed6b965-27bf-4598-8bca-b1b9a18fc48a",
   "metadata": {},
   "source": [
    "#### Input Perceptrons\n",
    "\n",
  ...
  {
   "cell_type": "markdown",
   "id": "b9f6b88a-1ad9-4bfa-bc3d-f534ae871601",
   "metadata": {},
   "source": [
    "### Sigmoid Neurons\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "<div align=\"center\">\n",
    "<img src=\"../../static/images/dl/string_of_numbers.png\" width=\"300px\" height=\"80px\">\n",
    "</div>\n",
    "\n",
    ...
    "\n",
    "<div align=\"center\">\n",
    "<img src=\"../../static/images/dl/three_layered_feedforward_network.png\" width=\"600px\" height=\"400px\">\n",
    "</div>\n",
    "\n",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "**Note:** Before moving to the next section you must learn about [Gradient Descent](../../ml/Gradient_Descent)."
   ]
  },
  {
   "cell_type": "markdown",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "**What’s happening here?** Let's say `sizes` is a list like `[784, 30, 10]` — meaning `784` input neurons, `30` hidden, `10` output.\n",
    "\n",
    "**Biases:**"
   ]
  },
  {
   ...
   "id": "d57e3ef0-439a-46b8-a0a8-eb9e8b62fd57",
   "metadata": {},
   "outputs": [],
   "source": [
    "self.biases = [np.random.randn(y, 1) for y in sizes[1:]]"
  ...
  {
   "cell_type": "markdown",
   "id": "9204cab0-8d86-4c39-b87a-19102746094d",
   "metadata": {},
   "source": [
    "`sizes[1:]` skips the input layer — input neurons don't have biases, only hidden and output layers do. For `[784, 30, 10]` this creates two bias vectors: shape `(30, 1)` and `(10, 1)`. Each neuron gets its own scalar bias, stored as a column vector so matrix math works cleanly.\n",
    "\n",
    "**Weights:**"
   ]
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "id": "bd8b2958-54f9-4987-8926-b71d4d6908d0",
   "metadata": {},
   "outputs": [],
   "source": [
  ...
  {
   "cell_type": "markdown",
   "id": "a67e13d1-968a-4571-8859-9b7dfdfd4911",
   "metadata": {},
   "source": [
    "`zip(sizes[:-1], sizes[1:])` pairs adjacent layers: `(784, 30)` and `(30, 10)`. For each pair, a weight matrix of shape `(y, x)` is created — so `(30, 784)` and `(10, 30)`. The shape is `(y, x)` not `(x, y)` because during the forward pass you compute `w @ a` where `a` is the activation column vector of the previous layer — the dimensions must align as `(y, x) @ (x, 1) = (y, 1)`.\n",
    "\n",
    "`np.random.randn` initializes from a standard normal distribution (mean=0, std=1). This is a basic initialization — in practice you'd scale it down (e.g. divide by `sqrt(x)`) to prevent activations from saturating at the start of training."
   ]
  },
  {
   "cell_type": "markdown",
   "id": "3097caf6-654f-4a5c-8c45-5a364bdd34eb",
   "metadata": {},
   "source": [
    "<div align=\"center\">\n",
    "<img src=\"../../static/images/dl/neural_network_weights_biases.svg\" width=\"650px\" height=\"650px\">\n",
    "</div>"
   ]
  },
  {
   "cell_type": "markdown",
   "id": "2c3b7dd3-3862-4f6e-87c4-d44fc8709eea",
   "metadata": {},
   "source": [
    "This shows `sizes = [3, 4, 2]`. The key things to see:\n",
    "\n",
    "The weight matrix `W₁` has shape `(4, 3)` — that's `(y, x)` where `y=4` hidden neurons and `x=3` input neurons. Every input neuron connects to every hidden neuron, so there are `3×4=12` weights. The line thickness represents the random weight magnitude from `np.random.randn`.\n",
    "\n",
    "The bias `b` nodes (coral) each feed into exactly one neuron — one scalar per neuron. The hidden layer gets a `(4,1)` bias vector, the output layer gets a `(2,1)` bias vector. Input neurons get no bias at all, which is why `sizes[1:]` skips index `0`.\n",
    "\n",
    "`zip(sizes[:-1], sizes[1:])` pairs `(3,4)` and `(4,2)` — one weight matrix per pair of adjacent layers.\n",
    "\n",
    "This allows us to use the elegant matrix equation:\n",
    "$$a' = \\sigma(wa + b)$$\n",
    "\n",
  ...
  },
  {
   ...
   "outputs": [
    ...
    },
    {
     ...
     "output_type": "stream",
     "text": [
      "/var/folders/k8/h1l_hrqs1gdfmyrtv74dvgb40000gn/T/ipykernel_8067/3083060216.py:51: VisibleDeprecationWarning: dtype(): align should be passed as Python or NumPy boolean but got `align=0`. Did you mean to pass a tuple to create a subarray type? (Deprecated NumPy 2.4)\n",
      "  training_data, validation_data, test_data = pickle.load(f,\n"
     ]
    },
    {
